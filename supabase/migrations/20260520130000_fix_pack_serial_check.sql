-- Fix transition_transfer_status: serial check was blocking ALL part-only items.
-- New rule: only block packing if the part has in-stock serials but none is assigned.
-- Parts with no serials in inventory (bulk/consumable) can pack freely.
...
begin
  ...
  -- Only block packing if the item's part has in-stock serials but none is assigned.
  -- Part-only (bulk/consumable) items with no serials in inventory are allowed through.
