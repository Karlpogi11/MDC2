create or replace function public.batch_upsert_sites(p_rows jsonb)
returns jsonb language plpgsql security definer as $$
declare
  v_errors jsonb := '[]'::jsonb;
  v_row jsonb; v_code text; v_name text; v_prefix text; v_address text;
begin
  ...
  for v_row in select * from jsonb_array_elements(p_rows) loop
    ...
    begin
      insert into public.sites (site_code, site_name, is_dc, invoice_prefix, address)
      values (v_code, v_name, coalesce((v_row->>'is_dc')::boolean, false), v_prefix, v_address)
