-- Fix batch_upsert_parts: include part_type (required NOT NULL column)
-- Derive from category if not provided: 'material' if category contains 'material', else 'product'
...
begin
  ...
  loop
    ...
    -- Derive part_type: explicit > category-based > default 'product'
    v_ptype := coalesce(
